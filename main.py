...
def play(speed):
    global flag
    print(f"You clicked on slow forward. Speed is {speed}")
    ...
    if flag:
        canvas.create_text(132,25,fill ="red",font="Times 22 bold",text="Decision pending")
    flag = not flag
...
btn = tkinter.Button(window, text="<<< Rewind(fast)", width=50,command=partial(play,-25))
btn.pack()

btn = tkinter.Button(window, text="<< Rewind(slow)", width=50,command=partial(play,-2))
btn.pack()
